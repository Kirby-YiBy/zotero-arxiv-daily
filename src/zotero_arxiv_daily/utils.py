...
def extract_tex_code_from_tar(file_path: str, paper_id: str) -> dict[str, str]:
    ...
    match len(bbl_file):
        ...
        case _:
            logger.debug(
                f"Cannot find main tex file of {paper_id} from bbl: There are multiple bbl files."
            )
            main_tex = None

    if main_tex is None:
        logger.debug(
            f"Trying to choose tex file containing the document block as main tex file of {paper_id}"
        )
    # read all tex files
    file_contents = {}
    for t in tex_files:
        ...
        if (
            ...
            and not any(w in t for w in ["example", "sample"])
        ):
            main_tex = t
    ...
    if main_tex is not None:
        ...
        logger.debug(
            f"Failed to find main tex file of {paper_id}: No tex file containing the document block."
        )
