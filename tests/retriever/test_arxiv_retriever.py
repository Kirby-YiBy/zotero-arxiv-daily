...
def test_convert_to_paper_uses_pdf_text_for_affiliations(config, monkeypatch):
    retriever = ArxivRetriever(config)
    raw_paper = SimpleNamespace(
        title="MegaTrain",
        authors=[SimpleNamespace(name="Author A")],
        summary="Abstract",
        pdf_url="https://arxiv.org/pdf/2604.05091v1",
        entry_id="https://arxiv.org/abs/2604.05091v1",
        source_url=lambda: "https://arxiv.org/e-print/2604.05091v1",
    )

    monkeypatch.setattr(
        arxiv_retriever, "extract_text_from_html", lambda paper: "HTML TEXT"
    )
    monkeypatch.setattr(
        arxiv_retriever, "extract_text_from_pdf", lambda paper: "PDF TEXT"
    )
    monkeypatch.setattr(
